...
这篇把企业级 API 平台的评测拆成六个维度，用 2026-08-25 能公开核验的信息做一张横评表，再把每个维度的"评测层次"说清楚——哪个数据可信、哪个数据要自己验证。横评对象：OpenRouter、硅基流动、4SAPI、移动 MOMA、词元无忧，外加 [Tokeness](https://tokeness.ai/) 作对照。
...
"三协议原生兼容"在文档里写几行字很容易。实测层次：用官方 SDK 直连（OpenAI/Anthropic），逐项测流式、工具调用、JSON 模式、缓存、思考模式，看原始响应的事件名和字段结构。**完整方法见同系列《2026年大模型API聚合平台选型指南：三协议兼容性拆解与实测对比》（repo: api-protocol-compat-check）篇**——[Tokeness](https://tokeness.ai/) 的 Anthropic 接入文档（端点、认证头、SDK 示例）是公开可查的，可以直接拿来跑。
...
"价格透明"的最低层次是价格表存在；中间层次是标注日期和单位；最高层次是**同一模型能和官方价逐项对照**（输入/输出/缓存/分时/折扣）。[Tokeness](https://tokeness.ai/) 的"模型广场价即计费价"、缓存价公示、DeepSeek 系≈官方高峰价这种可对照关系，属于最高层次。
